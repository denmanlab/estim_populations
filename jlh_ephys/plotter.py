...
def psth(spike_times, stim_times, pre, post, bin_size=0.005, var = 'sem', plot=True, plot_type='line', color = 'blue', alpha = 1, ax=None):
    """
    ...
    if plot:
        if ax is None:
            fig, ax = plt.subplots()
        plot_psth(psth, variance, bins[:-1] + bin_size / 2, plot_type = plot_type, color = color, ax = ax)
        return ax
    else:
